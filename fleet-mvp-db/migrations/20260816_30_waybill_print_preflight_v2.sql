...
begin
  ...
  blocking_count:=(case when header_ok then 0 else 1 end)
    ...
    +(case when strict_closed and not route_ok then 1 else 0 end)
    +(case when strict_closed and not fuel_ok then 1 else 0 end);
  ...
  return jsonb_build_object(
    'ui_version','waybill_print_preflight_v2',
    ...
    'checks',jsonb_build_array(
      ...
      jsonb_build_object('id','route','ok',route_ok,'blocking',strict_closed,'label',case when c.mileage_km is null then 'Пробег ещё не закрыт' when route_ok then 'Маршрут покрывает подтверждённый пробег' else 'Маршрут не покрывает подтверждённый пробег полностью' end),
      jsonb_build_object('id','fuel','ok',fuel_ok,'blocking',strict_closed,'label',case when fuel_ok then 'Заправки сходятся с учётом топлива' else 'Сумма заправок не сходится с учётом топлива' end),
