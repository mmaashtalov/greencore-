...
(async()=>{
  if(typeof cfg.supabaseUrl!=='string'||typeof cfg.publishableKey!=='string'||cfg.supabaseUrl.includes('__SUPABASE_')||cfg.publishableKey.includes('__SUPABASE_')) fail('Runtime-конфигурация production не подставлена при деплое.');
  let url;
  ...
  if(!cfg.publishableKey.startsWith('sb_publishable_')) fail('Не задан production Supabase publishable key.');

  await import('./prod-hardening-v1.js?v=20260817-4');
  const modules=[
  ...
  window.dispatchEvent(new CustomEvent('fleet:prod-bootstrap-ready',{detail:{version:'2026.08.17-bootstrap4'}}));
