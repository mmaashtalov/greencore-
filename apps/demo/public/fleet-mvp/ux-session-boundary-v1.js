const UXSB_VERSION='2026.08.15-session1';
...
let logoutReloadScheduled=false;
...
function isolateForRole(){const role=document.body.dataset.role,uid=userId();if(role==='driver'&&uid){restore(uid);return}if(role==='admin'){const owner=localStorage.getItem(KEYS.owner);if(owner)stash(owner);localStorage.removeItem(KEYS.owner);sessionStorage.removeItem(KEYS.ctx)}}
function beforeLogout(){const uid=userId()||localStorage.getItem(KEYS.owner);if(uid)stash(uid);localStorage.removeItem(KEYS.owner);sessionStorage.removeItem(KEYS.ctx);if(logoutReloadScheduled)return;logoutReloadScheduled=true;setTimeout(()=>location.reload(),40)}
