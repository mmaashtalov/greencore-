...
function arItemFor(home,id){return (home?.attention||[]).find(x=>x.id===id)||null}
function arTargetFor(home,id){return arItemFor(home,id)?.target||null}
function arPriorityMeta(item){const priority=Number(item?.priority);if(item?.tone==='danger')return{label:'Срочно',className:'urgent'};if(Number.isFinite(priority)&&priority<=2)return{label:'Сначала',className:'now'};if(Number.isFinite(priority)&&priority<=5)return{label:'Сегодня',className:'today'};return{label:'Далее',className:'next'}}
function arFilterLabel(filter){return({repair:'Только машины в ремонте',maintenance:'Только задачи по ТО',defect:'Только новые неисправности',attention:'Только требующие решения'})[filter]||'Отфильтровано по задаче с главной'}
