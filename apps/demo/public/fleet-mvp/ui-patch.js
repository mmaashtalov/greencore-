...
function compactHeader() {
  ...
  sub.textContent = `Демо · ${role}`;
  sub.dataset.uiPatch = PATCH_CFG.version;
}
function roleCandidates(employees, type) {
  const all = employees || [];
  const nonDrivers = all.filter(e => !/водитель/i.test(e.position || ''));
  if (type === 'senior') {
    const exact = all.filter(e => /старш.*машин/i.test(e.position || ''));
    return exact.length ? exact : nonDrivers;
  }
  const exact = all.filter(e => /ответствен.*эксплуатац|ответствен.*техник|начальник.*авто/i.test(e.position || ''));
  return exact.length ? exact : nonDrivers;
}
function fillEmployeeSelect(select, candidates, emptyLabel) {
  if (!select) return;
  const current = select.value;
  select.innerHTML = `<option value="">${emptyLabel}</option>` + candidates.map(e =>
    `<option value="${e.id}">${[e.rank,e.label,e.position].filter(Boolean).join(' · ')}</option>`
  ).join('');
  if (candidates.some(e => e.id === current)) select.value = current;
  else if (candidates.length === 1) select.value = candidates[0].id;
}
function makeCompactPeriod(form) {
  const from = form.elements.valid_from;
  const to = form.elements.valid_to;
  if (!from || !to || form.querySelector('.issue-period')) return;
  const fromField = from.closest('.field');
  const toField = to.closest('.field');
  const grid = fromField?.parentElement;
  if (!fromField || !toField || !grid) return;
  const details = document.createElement('details');
  details.className = 'issue-period';
  const summary = document.createElement('summary');
  const inner = document.createElement('div');
  inner.className = 'form-grid two issue-period-grid';
  details.append(summary, inner);
  inner.append(fromField, toField);
  grid.append(details);
  const update = () => { summary.textContent = `Срок: ${shortDateTime(from.value)} — ${shortDateTime(to.value)}`; };
  from.addEventListener('change', update);
  to.addEventListener('change', update);
  update();
}
async function hydrateIssueForm(form) {
  if (!form || form.dataset.uiPatch) return;
  form.dataset.uiPatch = PATCH_CFG.version;

  const vehicle = form.elements.vehicle_id;
  const driver = form.elements.driver_id;
  const trailer = form.elements.trailer_id;
  const submit = form.querySelector('button[type="submit"]');
  const hint = document.getElementById('issueHint');
  if (!vehicle || !driver) return;

  vehicle.id = 'issueVehicleV2';
  driver.id = 'issueDriverV2';
  makeCompactPeriod(form);
  ...
  async function refreshCompatibility() {
    ...
    const trailerId = trailer?.value || null;
    const previousDriver = driver.value;

    if (!vehicleId) {
      driver.innerHTML = '<option value="">Сначала выберите машину</option>';
    ...
    driver.disabled = true;
    if (submit) submit.disabled = true;
    if (hint) hint.textContent = 'Проверяем допуск водителя…';
    ...
    try {
      ...
      if (drivers.some(d => d.id === previousDriver)) driver.value = previousDriver;
      else if (ctx?.recommended_driver_id && drivers.some(d => d.id === ctx.recommended_driver_id)) driver.value = ctx.recommended_driver_id;
      else if (drivers.length === 1) driver.value = drivers[0].id;

      driver.disabled = drivers.length === 0;
      if (submit) submit.disabled = drivers.length === 0;
  ...
  vehicle.addEventListener('change', refreshCompatibility);
...
function scanUi() {
  ...
  if (issueForm) hydrateIssueForm(issueForm);
}
function scheduleScan() {
  if (scheduled) return;
  scheduled = true;
  queueMicrotask(scanUi);
}
